...
const APP_ENTITY =
  (process.env.NEXT_PUBLIC_VENTO_ENTITY?.toLowerCase() as LocalAppEntity) ?? "fogo";

const APP_CODE: CanonicalAppCode = APP_ENTITY === "default" ? "fogo" : APP_ENTITY;
...
export async function VentoShell({ children }: { children: React.ReactNode }) {
  ...
  if (user) {
    ...
    if (sharedDevice) {
      ...
      activeContextDetail = currentAppAllowed
      ...
      if (activeWorkContext?.siteId || activeWorkContext?.shiftId || activeWorkContext?.operationalRole) {
        ...
        activeContextDetail = operatingGate.description;
